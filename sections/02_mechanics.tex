...
\begin{frame}[t]
  ...
  \begin{align*}
    \executionHS = \left( \ResetMap, \mathscr{I}, \mathscr{C} \right),
  \end{align*}
  \vspace{-2em}
  \begin{itemize}
  \item $\ResetMap = \{0, 1, 2, \ldots\} \subseteq \Nat$ is a finite or infinite \blue{indexing set},
  \item $\mathscr{I} = \{I_{i} \}_{i \in \ResetMap}$ is a \blue{hybrid interval} where $I_{i} = [\tau_{i}, \tau_{i + 1}]$,
  \item $\mathscr{C} = \{c_{i} \}_{i \in \ResetMap}$ is a \blue{collection of solutions} of $\xf$, i.e., ${\dot c}_{i}\art = \xf(c_{i}\art)$ $\forall i \in \ResetMap$.
  \end{itemize}
  ...
  \only<2>{
    In addition, for every $i, i + 1 \in \ResetMap$, it is required that
    \begin{enumerate}
  ...
  \only<1>{
    \begin{columns}
      ...
      \begin{align*}
        ...
        \uu_3 &=-\ck_{d} (\dot \vartheta_{T}^{a})\\
        &\hspace{1.8em} -\ck_{p} (\vartheta_{T}^{a} - \vartheta_{T}^{d})
      \end{align*}
      \column{1.5in}
      \begin{figure}
        \centering
